...
class Paginator(discord.ui.View):
	...
	async def send(self, embeds, reply=False):
		...
		self.button_next = discord.utils.get(self.children, custom_id="button_next")
		self.button_last = discord.utils.get(self.children, custom_id="button_last")
		self.button_previous = discord.utils.get(self.children, custom_id="button_previous")
		self.button_first = discord.utils.get(self.children, custom_id="button_first")
		self.button_index = discord.utils.get(self.children, custom_id="button_index")

		self.button_index.label = f"Page 1 of {self.length}"
	...
	@button(style=discord.ButtonStyle.secondary, label="\u00AB", custom_id="button_first")
	async def button_first_callback(self, interaction, button):
		if self.index == 0:
			return
	...
	@button(style=discord.ButtonStyle.secondary, label="\u2039", custom_id="button_previous")
	async def button_previous_callback(self, interaction, button):
		if self.index == 0:
			return
	...
	@button(style=discord.ButtonStyle.red, emoji="\U0001f6d1", label=f"Page 1 of ...", custom_id="button_index")
	async def button_index_callback(self, interaction, button):
		await interaction.message.delete()
		self.stop()

	@button(style=discord.ButtonStyle.secondary, label="\u203A", custom_id="button_last")
	async def button_next_callback(self, interaction, button):
		if self.index == self.length-1:
			return
	...
	@button(style=discord.ButtonStyle.secondary, label="\u00BB", custom_id="button_next")
	async def button_last_callback(self, interaction, button):
		if self.index == self.length-1:
			return
